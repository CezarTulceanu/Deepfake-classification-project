...
Original file is located at
    https://colab.research.google.com/drive/1omE91mXgGmd24Bl9yvW7rCfIny6g8Nm3
"""
...
#am accentuat augmentarea datelor
...
def create_optimized_cnn(input_shape, num_classes, l2_lambda=1e-5):
    ...
    x = layers.Conv2D(96, 3, padding='same',
                      kernel_regularizer=regularizers.l2(l2_lambda))(x)
    x = layers.ReLU()(x)
    x = layers.MaxPooling2D()(x)
    x = layers.Dropout(0.1)(x)

    x = layers.Conv2D(192, 3, padding='same',
                      kernel_regularizer=regularizers.l2(l2_lambda))(x)
    x = layers.ReLU()(x)
    x = layers.MaxPooling2D()(x)
    ...
    skip = layers.Conv2D(32, 3, padding='same',
                         kernel_regularizer=regularizers.l2(l2_lambda))(inputs)
    skip = layers.ReLU()(skip)
    skip = layers.AveragePooling2D(pool_size=(16, 16), strides=(16, 16))(skip)
    skip = layers.Flatten()(skip)

    #Am extins foarte mult straturile de final ale modelului pentru a-i permite sa traga concluzia mai bine
...
lr_schedule = ExponentialDecay(
    ...
    decay_steps=10000,
    decay_rate=1.7,         # Am facut rata de invatare sa creasca si mai repede (Ma plictiseam; Antrenarea dura deja ~2h)  
    staircase=False
)
...
#I-am dat mai multa rabdare pentru ca modelul era instabil si puteam fi pacalit de o scadere scurta in acuratete
